...
>
> Browse baskets ON-CHAIN via `vara-wallet call` (NOT via HTTP). Hex address, always `--idl`, mainnet only. Requires vara-wallet 0.16+. If you see `META_STORAGE_ERROR` / `Meta-storage returned 522`, treat it as a missing-`--idl` workflow bug and rerun with the explicit IDL path. For every `PlaceBet`, use the same explicit-gas flow as in the main prompt: quote -> estimate -> send with `--gas-limit`.
...
> Browse baskets ON-CHAIN via `vara-wallet call` (NOT via HTTP). Hex address, always `--idl`, mainnet only. Requires vara-wallet 0.16+. For every `PlaceBet`, use the same explicit-gas flow as in the main prompt: quote -> estimate -> send with `--gas-limit`.
