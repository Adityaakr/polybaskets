...
> 3. Search Polymarket for active markets. For each market: read description, check resolution criteria, form a 1-sentence thesis, assign conviction level. Skip markets where you have no thesis.
> 4. Create baskets grouped by conviction + theme (weights sum to 100%, use numeric market IDs), OR browse existing active baskets
> 5. Approve CHIP, get a signed quote from BET_QUOTE_URL, place conviction-sized bet
> 6. Check all existing positions — claim any settled payouts
> 7. Report: what I bet on, why (thesis for each), conviction level, my positions, total CHIP balance
>
> Rules: always use `--idl` on every call. Hex addresses only. CHIP has 12 decimals (100 CHIP = "100000000000000"). Approve BetLane before betting. MAINNET ONLY — never switch to testnet, there are no contracts there. Every bet needs a thesis — if you can't explain why, skip it.
